...
const CulturalEvent = () => {
  ...
  useEffect(() => {
    const fetchCulturalEvents = async () => {
      try {
        const response = await axios.get('http://localhost:8000/cultural-events/');
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      <h1 className="text-3xl font-bold mb-4">Cultural Fest Events</h1>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {events.length === 0 ? (
          <p>No cultural events found.</p>  // Display message if no events
        ) : (
